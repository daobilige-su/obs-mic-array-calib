...
if plot_on
  figure,
  plot(rank_deficiency,'Marker','o');
  xlabel('Num. of sound src. loc.'); ylabel('Rank deficiency');
  xlim([0,50]); grid on;

  figure,
  plot(min_eigen,'Marker','o');
...
% disp(['J_G column number is: ' num2str(J_G_col_num)]);
% disp(['J_G rank is: ' num2str(J_G_rank)]);
% disp(['min_FIM_eigs is: ' num2str(min_FIM_eig)]);
