...
function h = drawellipse_3d(x,a,b,color,axis,axis_value)
...
% Plot
if axis == 'z'
    h = plot3(p(1,:),p(2,:),ones(size(p(1,:)))*axis_value,'Color',color);
elseif axis == 'y'
    h = plot3(p(1,:),ones(size(p(1,:)))*axis_value,p(2,:),'Color',color);
elseif axis == 'x'
    h = plot3(ones(size(p(1,:)))*axis_value,p(1,:),p(2,:),'Color',color);
else
    disp('error in drawellipse_3d axis sepecification');
end
