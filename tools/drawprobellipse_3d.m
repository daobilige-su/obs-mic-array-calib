...
function drawprobellipse_3d(x,C,alpha,color)

x_z = x(1:2);
C_z = C(1:2,1:2);
drawprobellipse_on_plane(x_z,C_z,alpha,color,'z',x(3));

x_y = x([1,3]);
C_y = C([1,3],[1,3]);
drawprobellipse_on_plane(x_y,C_y,alpha,color,'y',x(2));

x_x = x(2:3);
C_x = C(2:3,2:3);
drawprobellipse_on_plane(x_x,C_x,alpha,color,'x',x(1));

end

function h = drawprobellipse_on_plane(x,C,alpha,color,axis,axis_value)
...
% Draw ellipse
h = drawellipse_3d(x,a,b,color,axis,axis_value);
